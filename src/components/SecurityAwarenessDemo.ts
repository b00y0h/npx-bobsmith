...
export const SecurityAwarenessDemo = async (): Promise<void> => {
  console.log("\n");
  const spinner = await createAnimatedSpinner(
  ...
  console.log("\n");
  console.log(
    chalk.red.bold("🚨 SECURITY ALERT: Recently visited sites found:")
  );
  console.log("\n");
  ...
  for (const item of fakeBrowserHistory) {
    console.log(
      chalk.yellow(
        `[${item.timestamp}] ${item.browser}: ${chalk.cyan(item.site)}`
      )
    );
    await sleep(300);
  }
  ...
  console.log("\n");
  await animateText(
    chalk.red.bold("⚠️ IMPORTANT SECURITY WARNING ⚠️"),
    CONFIG.THEME.ANIMATION_SPEED.FAST
  );

  const message = [
    chalk.white(
      "This is a security demonstration. A malicious NPX script could potentially:"
    ),
    ...
    "",
    chalk.white.bold(
      "Always be cautious when running unknown scripts from the internet."
    ),
    ...
    chalk.white(
      `Contact me at ${chalk.green(
        CONFIG.URLS.EMAIL || "email@example.com"
  ...
  console.log("\n");
  await animateText(
    chalk.cyan("Press any key to continue to the business card..."),
    CONFIG.THEME.ANIMATION_SPEED.FAST
  );

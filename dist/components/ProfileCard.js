...
export const ProfileCard = async () => {
    const cardData = {
        ...
        education: `${chalk.white("Graduate Of")} ${morning(CONFIG.PERSONAL_INFO.EDUCATION)}`,
        github: CONFIG.URLS.GITHUB
            ? `${chalk.white("{")} ${chalk.gray("github.com/")}${chalk.green("b00y0h")} ${chalk.white("}")}`
            : "",
        linkedin: CONFIG.URLS.LINKEDIN
            ? `${chalk.white("{")} ${chalk.gray("linkedin.com/in/")}${chalk.blue("bobbysmith")} ${chalk.white("}")}`
            : "",
    ...
    cardLines.push("", `📇 Card:      ${cardData.npx}`, "", passion("🚀 Available for exciting opportunities and collaborations!"), cristal("💭 Let's connect and create something amazing together!"));
